...
# 运行配置向导（前台运行，配置完成后程序会自动继续运行）
echo -e "${GREEN}✅ 下载完成，启动配置向导...${NC}"

# 关键修改：使用 exec 替换当前 shell 进程，或者使用 nohup 配合 & 
# 方案1：直接运行（配置向导完成后程序会继续运行，但脚本会等待）
/usr/local/aegisproxy/AegisProxy

# 脚本执行完毕，但程序还在运行，不会被杀死
echo -e "${GREEN}✅ 安装完成！${NC}"
echo -e "${GREEN}✅ AegisProxy 已在后台运行${NC}"
echo -e "${YELLOW}💡 查看进程: ps aux | grep AegisProxy${NC}"
echo -e "${YELLOW}💡 停止程序: pkill AegisProxy${NC}"
